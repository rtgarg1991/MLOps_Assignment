...
def main():
    ...
    # Define Paths
    paths = [
        ("raw", f"data/raw/pr-{args.pr_number}/raw.csv", "data/raw/production/latest.csv"),
        ("processed", f"data/processed/pr-{args.pr_number}/processed.csv", "data/processed/production/latest.csv"),
        ("features", f"data/features/pr-{args.pr_number}/features.csv", "data/features/production/features.csv")
    ]
    ...
    for data_type, source, dest in paths:
        ...
        log_promotion_to_bq(project_id, int(args.pr_number), data_type, f"gs://{args.bucket}/{dest}")
